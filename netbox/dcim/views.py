...
class PathTraceView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        api_viewname = (
            f"{path.origin_type.app_label}-api:{path.origin_type.model}-trace"
        )
        svg_url = (
            f"{reverse(api_viewname, kwargs={'pk': path.origins[0].pk})}?render=svg"
        )
                ...
                extra=(
                    (
                        Location.objects.restrict(request.user, "view").filter(
                            site__region__in=regions
                        ),
                        "region_id",
                    ),
                    (
                        Rack.objects.restrict(request.user, "view").filter(
                            site__region__in=regions
                        ),
                        "region_id",
                    ),
                    ...
                    (
                        CircuitTermination.objects.restrict(
                            request.user, "view"
                        ).filter(_region__in=regions),
                        "region_id",
                    ),
                    (
                    ...
                    (
                        WirelessLAN.objects.restrict(request.user, "view").filter(
                            _region__in=regions
                        ),
                        "region_id",
                    ),
        ...
        return {
            "related_models": self.get_related_models(
                ...
                extra=(
                    ...
                    (
                        VLAN.objects.restrict(request.user, "view").filter(
                            site__group__in=groups
                        ),
                        "site_group_id",
                    ),
                    (
                        ASN.objects.restrict(request.user, "view")
                        .filter(sites__group__in=groups)
                        .distinct(),
                        "site_group_id",
                    ),
                    (
                    ...
                    (
                        CircuitTermination.objects.restrict(
                            request.user, "view"
                        ).filter(_site_group__in=groups),
                        "site_group_id",
                    ),
                    (
                        Cluster.objects.restrict(request.user, "view").filter(
                            _site_group__in=groups
                        ),
                        "site_group_id",
                    ),
                    (
        ...
        return {
            "related_models": self.get_related_models(
                ...
                extra=(
                    ...
                    (
                        Circuit.objects.restrict(request.user, "view")
                        .filter(terminations___site=instance)
                        .distinct(),
                        "site_id",
                    ),
                    # Handle these relations manually to avoid erroneous filter name resolution
...
class RackView(GetRelatedModelsMixin, generic.ObjectView):
    queryset = Rack.objects.prefetch_related(
        "site__region", "tenant__group", "location", "role"
    )
...
class RackNonRackedView(generic.ObjectChildrenView):
    ...
    tab = ViewTab(
        ...
        badge=lambda obj: obj.devices.filter(
            rack=obj, position__isnull=True, parent_bay__isnull=True
        ).count(),
        weight=500,
...
class ManufacturerView(GetRelatedModelsMixin, generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        return {
            "related_models": self.get_related_models(
                request, instance, [InventoryItemTemplate]
            ),
        }
...
@register_model_view(
    ConsoleServerPortTemplate, "bulk_delete", path="delete", detail=False
)
...
class InventoryItemTemplateCreateView(generic.ComponentCreateView):
    ...
    def alter_object(self, instance, request):
        ...
        if component_type and component_id:
            ...
            instance.component = get_object_or_404(
                content_type.model_class(), pk=component_id
            )
...
class DeviceView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        return {"vc_members": vc_members, "svg_extra": f"highlight=id:{instance.pk}"}
    ...
    def get_children(self, request, parent):
        return (
            ...
            .prefetch_related(
                ...
                Prefetch(
                    "member_interfaces",
                    queryset=Interface.objects.restrict(request.user),
                ),
            )
        )
...
class DeviceVirtualMachinesView(generic.ObjectChildrenView):
    ...
    tab = ViewTab(
        ...
        badge=lambda obj: VirtualMachine.objects.filter(
            cluster=obj.cluster, device=obj
        ).count(),
        weight=2200,
        hide_if_empty=True,
    ...
    def get_children(self, request, parent):
        return self.child_model.objects.restrict(request.user, "view").filter(
            cluster=parent.cluster, device=parent
        )
...
@register_model_view(
    ConsoleServerPort, "bulk_disconnect", path="disconnect", detail=False
)
...
# Trace view
register_model_view(ConsoleServerPort, "trace", kwargs={"model": ConsoleServerPort})(
    PathTraceView
)
...
class InterfaceView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        vdc_table = tables.VirtualDeviceContextTable(
            data=instance.vdcs.restrict(request.user, "view").prefetch_related(
                "device"
            ),
            exclude=(
                "tenant",
        ...
        bridge_interfaces = Interface.objects.restrict(request.user, "view").filter(
            bridge=instance
        )
        ...
        child_interfaces = Interface.objects.restrict(request.user, "view").filter(
            parent=instance
        )
        child_interfaces_table = tables.InterfaceTable(
            child_interfaces, exclude=("device", "parent"), orderable=False
        )
...
class CableEditView(generic.ObjectEditView):
    ...
    def alter_object(self, obj, request, url_args, url_kwargs):
        ...
        a_terminations_type = CABLE_TERMINATION_TYPES.get(
            request.POST.get("a_terminations_type")
            or request.GET.get("a_terminations_type")
        )
        b_terminations_type = CABLE_TERMINATION_TYPES.get(
            request.POST.get("b_terminations_type")
            or request.GET.get("b_terminations_type")
        )

        if obj.pk:
            if not a_terminations_type and (
                termination_a := obj.terminations.filter(cable_end="A").first()
            ):
                a_terminations_type = termination_a.termination._meta.model
            if not b_terminations_type and (
...
class VirtualChassisEditView(ObjectPermissionRequiredMixin, GetReturnURLMixin, View):
    ...
    def post(self, request, pk):
        ...
        if vc_form.is_valid() and formset.is_valid():
            virtual_chassis._changelog_message = vc_form.cleaned_data.pop(
                "changelog_message", ""
            )
    ...
    def post(self, request, pk):
        ...
        if member_select_form.is_valid():
            ...
            if membership_form.is_valid():
                ...
                if "_addanother" in request.POST and safe_for_redirect(
